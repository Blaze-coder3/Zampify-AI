...
viewer_path = r"C:\Users\A Syed Khwaja\OneDrive\Desktop\Zampify-AI\frontend\components\DocumentViewer.tsx"
...
export default function DocumentViewer({ invoice }: DocumentViewerProps) {
  ...
  // PyMuPDF (digital) uses 72 DPI (~600x800). Tesseract (scanned) used 300 DPI (~2480x3508).
  const scale = invoice.extraction_method?.includes("scanned") ? 800 / 2480 : 800 / 595;
    ...
    <div className="flex-1 bg-slate-100 flex flex-col overflow-hidden relative border-r border-slate-200">
      ...
      <div className="flex-1 overflow-auto bg-slate-200 p-8 flex justify-center items-start relative">
        <div className="bg-white shadow-xl relative" style={{ width: '800px', minHeight: '1050px' }}>
          ...
          <div className="absolute inset-0 p-12 pointer-events-none opacity-50 grayscale">
...
print("Patched DocumentViewer.tsx")
