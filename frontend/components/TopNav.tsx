...
import { Search, Bell, HelpCircle, Settings, ChevronDown, Check, ShieldAlert } from "lucide-react";
...
export default function TopNav() {
  ...
  return (
    <header className="h-16 bg-[#0a1128] border-b border-gray-800 flex items-center justify-between px-6 sticky top-0 z-50 w-full transition-all">
                ...
                {(Object.entries(USERS) as [Role, typeof user][]).map(([roleKey, u]) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${user.role === roleKey ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300'}`}>
                        {u.initials}
                      </div>
                      <div>
